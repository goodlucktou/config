...
func (c *Config) reload(){	// 启动定时器 每5秒重新加载一次配置文件
...
func (c *Config) parse()(m map[string]string,err error){	// 读文件并或将文件中的数据以k/v的形式存储到map中
	...
	for{		
		...
		if errRet == io.EOF{// 表示读到文件的末尾
			break
		}
		if errRet != nil{	// 表示读文件出问题
			err = errRet
			return
		}
		lineNo++
		line = strings.TrimSpace(line) // 取出空格
		if len(line) == 0 || line[0] == '\n' || line[0] == '+' || line[0] == ';'{	// 当前行为空行或者是注释行等
			continue
		}
...
func (c *Config) GetInt(key string)(value int,err error){	// 根据int获取
	c.rwLock.RLock()
...
func (c *Config)GetIntDefault(key string,defval int)(value int){ // 默认值
	c.rwLock.RLock()
	defer c.rwLock.RUnlock()
	str,ok:=c.data[key]
	if !ok{
...
func (c *Config) GetString(key string)(value string,err error){	// 根据字符串获取
	c.rwLock.RLock()
	defer c.rwLock.RUnlock()
	value,ok := c.data[key]
	if !ok{
